      ...
      <Space direction="vertical" size={16} style={{ width: '100%' }}>
        ...
        <Card size="small" title="六、候选池与资金分配">
          <Paragraph>
            ...
            盘中每 15 分钟运行一次资金分配:对池内候选重新打分
            (档位 × 置信度 × 时效衰减 × 来源分 × 宏观与行业乘数),解决多空冲突后按分数排序,
            只对排名最高的少数候选做出最终交易决策。这样:
          </Paragraph>
          <ul>
            <li>资金优先给最高质量的信号,而不是先到先得;</li>
            <li>资金不足时高分候选留池等待,资金释放后自动复评;</li>
            <li>休市期间信号只累积、不交易,开盘后第一轮统一清算隔夜候选;</li>
        ...
        <Card size="small" title="八、仓位规模与风控审批">
          ...
          <ul>
            ...
            <li>最多同时持有 10 只股票,每天最多新开 3 个仓位;</li>
        ...
        <Card size="small" title="九、卖出、止损与持仓管理">
          <ul>
            <li>
              <Text strong>利空即卖</Text>:持仓股票出现高档位利空信号时走即时卖出流程(卖出不入池、不受预算限制);
            </li>
            <li>
              <Text strong>止损/止盈</Text>:每笔买入都带 AI 设定的止损(限定 3%~15%)与止盈(限定 5%~30%),
              盘中持续监控,触线即全仓卖出;
            </li>
            <li>
              <Text strong>移动止损</Text>:股价创出建仓后新高时,止损线按原始止损距离同步上移、只升不降,锁住浮盈;
            </li>
            <li>
              <Text strong>每日持仓复查</Text>:每个交易日 AI 复盘全部持仓,
              买入逻辑已失效的仓位会被卖出或收紧止损;
            </li>
            <li>休市时段产生的卖出信号挂单排队,下一次开盘自动成交;盘前盘后按真实盘外价格即时成交。</li>
          </ul>
        </Card>
