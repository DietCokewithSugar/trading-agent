 ...
 * analyst(032,回测分析缓存键):**修改 ANALYST_SYSTEM_PROMPT 文本时必须 +1**——
 * backtest_analyses 按 (url, symbol, prompt_version) 命中缓存,不升版会把旧 prompt
 * 的分析结果当成新 prompt 的缓存直接复用。
 */
export const PROMPT_VERSIONS = { trader: 1, 'risk-officer': 1, analyst: 1 };
...
4. 正文可能是 SEC 监管文件(如 8-K)的摘录,开头带 [SEC Form 8-K | Items: ...] 元数据头;条目编码提示事件类型(如 1.03=破产、2.02=业绩结果、4.02=财报不可依赖、5.02=高管变动),监管文件语气中性刻板,请按事件实质而非行文语气判断方向与程度。
5. ${UNTRUSTED_NOTE}`;

/**
 * 用 DeepSeek 分析一条新闻的利好/利空与档位。
 * purpose 可覆盖(默认 'analyst'):回测重析历史新闻传 'backtest-analyst',
 * 管理页 LLM 用量按用途分桶时不与实盘分析混桶。
 */
export async function analyzeArticle(article, { purpose = 'analyst' } = {}) {
  const user = [
...
- 若持仓带有"同期SPY涨跌百分比",请相对大盘评判表现:持仓涨幅明显落后同期 SPY 说明
  论点未带来超额收益(大盘红利不是信号质量),持续跑输者应更倾向卖出;
...
- 若给出"同期SPY涨跌百分比/超额收益百分比",必须按超额收益(alpha)评判,而非绝对盈亏(beta):
  大盘普涨日跑输 SPY 的盈利不算成功,大盘暴跌日跌幅小于 SPY 的亏损未必是决策错误——
  教训应针对超额收益的来源或缺失,不要把行情红利当信号质量;
  ...
  const user = JSON.stringify(
    {
      ...
      // 同持仓期大盘基准(可得时):评判 alpha 而非 beta
      ...(spyReturnPercent !== null && spyReturnPercent !== undefined
        ? { 同期SPY涨跌百分比: spyReturnPercent, 超额收益百分比: excessReturnPercent }
        : {}),
    },
    null,
    1
