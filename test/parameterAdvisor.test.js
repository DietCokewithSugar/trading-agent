...
test('腾位孪生对照(025):孪生变体自动进入影子规则,文案指向基础变体对比', () => {
  const now = Date.now();
  const started = new Date(now - 30 * 86400_000).toISOString();
  const win = evaluateShadowRules({
    variants: [{ variant: 'wide_bracket_rotation', started_at: started, window_return_pct: 9 }],
    actualReturnPct: 3,
    now,
  });
  const adj = win.suggestions.find((s) => s.id === 'shadow_wide_bracket_rotation');
  assert.equal(adj?.level, 'adjust');
  assert.ok(/wide_bracket/.test(adj.suggestion), '建议要求与基础变体同窗对比');
});
