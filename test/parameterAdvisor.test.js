...
test('影子组合对照:显著跑赢 → adjust,显著跑输 → ok,样本期不足/差异小 → 沉默', () => {
  ...
  const mk = (pnl) => [{ variant: 'no_macro_filter', started_at: started, pnl_percent: pnl }];
  ...
  const young = evaluateShadowRules({
    variants: [{ variant: 'no_macro_filter', started_at: new Date(now - 3 * 86400_000).toISOString(), pnl_percent: 9 }],
    actualReturnPct: 0,
    now,
  });
