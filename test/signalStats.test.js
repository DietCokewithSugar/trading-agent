...
test('采样计划:默认窗口保留 recent + 已成熟 1d/5d,总预算不超上限', () => {
  const plan = buildSamplingPlan({ days: null, maxRows: 5000 });
  assert.deepEqual(
    plan.map((p) => p.bucket),
    ['recent', 'matured_1d', 'matured_5d']
  );
  assert.equal(
    plan.reduce((sum, p) => sum + p.maxRows, 0),
    5000
  );
  assert.ok(plan.find((p) => p.bucket === 'matured_1d')?.maxRows > 0);
  assert.ok(plan.find((p) => p.bucket === 'matured_5d')?.maxRows > 0);
});

test('采样计划:短窗口自动降级,避免无意义成熟口径查询', () => {
  assert.deepEqual(
    buildSamplingPlan({ days: 1, maxRows: 3000 }).map((p) => p.bucket),
    ['recent']
  );
  assert.deepEqual(
    buildSamplingPlan({ days: 3, maxRows: 3000 }).map((p) => p.bucket),
    ['recent', 'matured_1d']
  );
});
