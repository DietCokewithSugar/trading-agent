...
| 来源层级 | 评分 | 示例 |
|---------|------|------|
| 监管披露(官方直抓) | 1.00 | SEC EDGAR 8-K 原始文件(仅 `sec-filings` 渠道直抓,见下) |
| 权威通讯社 / 监管 | 0.95 | Reuters、AP、SEC(经媒体/聚合渠道转发的 sec.gov 链接) |
...
### SEC EDGAR 监管文件源(8-K,最高可信事件源)

在聚合 API / RSS 之上,系统直接轮询 **SEC EDGAR 官方接口**(免密钥,仅需按公平访问政策申明 `SEC_USER_AGENT`)的全市场「最新提交」实时流,发现所有上市公司的 8-K 重大事项公告(026 迁移):CIK→ticker 官方映射过滤掉基金/私有主体,再抓取每份新 filing 的主文档提取正文与事项条目编码(1.03=破产、2.02=业绩、5.02=高管变动等),只有命中重大条目白名单(`SEC_8K_ITEM_WHITELIST`)的才进入分析管线,例行披露直接丢弃。官方直抓的 8-K 按满分 1.00「监管披露」档计信——但 8-K 依然是**公司自述**:孤立 8-K 的利好信号与新闻稿共享 `PRESS_BULLISH_PENALTY` 门槛折价(媒体跟进报道通过既有交叉确认机制解锁),利空信号则按满分直接生效(公司很少自曝坏消息)。定位是"确认、校验、重大事件发现",不是"看到 8-K 就买"。
...
**展示主账本切换**(024 迁移,管理员页):开启后仪表盘主视图(净值/持仓/净值曲线,以及**关键指标与交易记录**)整体切换为**券商模拟账户**的真实撮合数据——累计收益率/今日盈亏/夏普/最大回撤按该账户的对照快照净值序列计算(盈亏基线为最早一条快照净值),已实现盈亏/胜率与交易列表来自该账户的镜像成交(按加权平均成本在服务端配对推导,与内部账本同口径);券商取数失败自动回退内部账本;每个账户的持仓、盈亏与收益率完全独立。交易引擎与决策仍完全基于内部模拟账本。需已配置券商模拟账户 API key。
...
行情商 profile 不是标的属性的权威事实源(ETF 检查还因字段常缺失而故意 fail-open)。系统直接轮询**交易所官方符号目录**(Nasdaq Trader Symbol Directory,免密钥,含 Nasdaq-listed 与 other-listed 全市场证券)构建 `symbol_reference` 名录(028 迁移),准入门在原有检查之上叠加五道确定性校验:**名录存在性**(不在名录=疑似 OTC/已退市,拒买)、**测试标的**(Test Issue)、**财务异常状态**(Financial Status D/E/Q 等=退市风险/破产程序,拒买)、**ETF 标记**(官方数据,补上 fail-open 的洞)、**上市交易所**(与行情商 profile 双源都须过白名单)。名录未就绪时整组跳过(可加性增强),残缺下载绝不换表(尾行完整性 + 行数下限双守卫)。

**停牌守护**:官方 Trading Halts feed 盘中轮询(`HALT_POLL_SECONDS`,默认 60s),停牌期间——**新买入禁止**(transient 拒绝,候选留池/挂单保留,复牌自动重试)、**持仓暂停止损/止盈/持有超时**(停牌中的报价是停牌前最后一笔,按 stale price 强平不真实;复牌后首个 tick 按真实跳空价执行)、**新闻利空卖单挂入开盘队列**(复牌后按真实价成交,与真实市场行为一致)、持仓表带**停牌标记**。守护是 fail-open 设计:feed 故障/数据陈旧时自动退化为无守护的现状,绝不因数据问题误拦交易。
...
- **净值对照**:券商账户 equity vs 内部组合净值,每 `BROKER_SNAPSHOT_SECONDS`(默认 30 秒)一条快照,休市时段自动降回 10 分钟一条;
- 长期未成交/过期的对照单本身就是信号——该价格在真实盘口拿不到。

**未成交跟进(027)**:镜像单未成交不再一弃了之(否则卖单过期会在券商账户滞留孤儿持仓、买单过期永不建仓,两账本永久分歧)——休市时段的镜像单先落"待开盘"(deferred),开盘后以实时价挂单;在途单过期/被撤后按策略重挂(`BROKER_MIRROR_MAX_RETRIES`,偏差基线始终是原内部成交价,追单成本如实入账):**卖单保证收敛**(限价重挂耗尽后升级一次市价单),**买单限时追单**(`BROKER_MIRROR_BUY_RETRY=chase`,上行漂移超过 `BUY_PRICE_DRIFT_ABORT_PERCENT` 或次数用尽即放弃,不以离谱价格追高污染对照数据);在途限价单超过 `BROKER_MIRROR_REPRICE_MINUTES`(默认 15 分钟)未成交会被**主动撤单重挂**(否则 day 单要等收盘过期才进重挂链,出场时效落后账本数小时);另有约每 30 分钟一轮的**对账清理**(`BROKER_MIRROR_RECONCILE`),自动平掉"券商持有但对账基准账本(内部账本,或该账户绑定的影子变体)已不持有"的滞留仓位。**镜像账户按现金账户语义运作**:买单先过现金约束(可用现金扣除在途买单占用;不足但有在途卖单回款可期则顺延,否则放弃),绝不动用保证金。

「消融实验」页的"券商模拟对照账本"卡片展示以上统计(`GET /api/broker-mirror`,仅统计 env 默认账户的实盘镜像)。**多账户(025)**:管理页可添加任意多个券商模拟账户(添加时校验凭据,secret 只存服务端、无公开读 RLS),每个账户可配置用途——「实盘镜像」与默认账户并行,或绑定某个消融变体:该变体的影子买卖以 marketable 限价单在该账户真实盘口撮合,让消融实验也有真实 NBBO 的对照账本;删除账户保留历史对照单。纯观测层:不配置 key 或未执行 021 迁移时整体停用,绝不影响交易主链路;管理重置会同步撤单、清空券商模拟持仓。注意免费数据档为单一交易所盘口(IEX),小盘股撮合价代表性有限;准入门槛(市值/成交额)已过滤掉大部分受影响标的。
...
- 后台任务自动回填三个口径的前瞻收益:信号后 1 小时(实时价,休市窗口错过则缺省)、信号日后第 1 / 第 2 个交易日收盘价(2d ≈48 小时,与持有上限对齐的决策口径,031 迁移;旧 5d 口径与 ±2%/48h 策略盈亏几乎无关且成熟期太长,已停止回填,列保留历史数据)。1d/2d 按**两条独立到期队列**回填(1d 满 24 小时、2d 满 72 小时才进队,最老优先),查不到日线的股票(退市/停牌)进入冷却名单不再空耗每轮预算——避免"毒丸行"把回填吞吐拖死;
- 统计样本只计入**至少回填了一个口径**的有效信号——刚产生、前瞻收益尚未到期的新信号不占采样预算(默认 20000 条,`SIGNAL_STATS_MAX_ROWS` 可调),覆盖窗口按有效数据计;
- **实盘兑现**(±2%/48h 策略口径):每笔平仓按"同票最近一笔买入"回溯源信号,按档位/来源分桶统计 止盈/止损/持有超时 的离场分布、胜率(Wilson 95% CI)与已实现盈亏——止盈占比高说明该类信号强度足以在时限内兑现 +2%,止损过半说明信号与固定敞口不匹配,这是当前策略下最直接的信号质量度量(1h 看即时反应、1d 次日定型、2d 对齐 48 小时持有上限);
...
- **时间窗口与样本透明**(017 后续):页面可切 7 天 / 30 天 / 全部(`?days=`),后端分页拉取突破单次 1000 行上限(超过采样上限时截断并明示实际覆盖的起点);每个命中率都带 **Wilson 95% 置信区间**,且仅在区间整体高于/低于 50% 时才着色——区间跨过 50% 说明样本不足以下结论,防止 20 条样本就调参;
...
- **前端「宏观」页**:当前环境与生效参数、经济日历与黑窗状态、宏观事件表;顶部**宏观环境日历热力图**(绿=风险偏好 / 红=避险·冲击 / 灰=中性,深浅按风险分)配日期筛选,可回看任意历史日——当日环境按当日事件用与实时相同的聚合规则回溯推算(`/api/macro?date=`,无 regime 历史表,序列由 `/api/macro/history` 按日推算并缓存 10 分钟),经济日历按需拉取该日实际发布值,宏观事件表同步限定该日;每笔成交记录当时的宏观环境标签。
...
**腾位孪生(025)**:上表每个可交易变体都有一个「+止盈腾位」孪生变体(immediate_trade 的孪生即 immediate_rotation)——与基础变体唯一差异是现金不足时先全仓卖出腾位选仓(trailing 系按浮盈比例最高,其余按最接近止盈价)再重试买入;孪生与基础变体的同窗差即腾位机制在该策略下的净贡献。若「无风控官」长期跑输实盘,说明风控官的否决在创造价值;若「无宏观过滤」跑赢,说明宏观层过度拦截;「信号即时成交」对比实盘量化候选池+LLM 决策链的净价值;「信号等权」对比实盘检验 LLM 仓位是否有效。**三个出场消融变体(023)补上了此前唯一没被消融的层**:实盘的信号驱动卖出(新闻利空/持仓复查/止盈腾位)照样镜像给它们,但实盘自身的止损/止盈/持有超时卖出**不镜像**——出场变体按自己的规则离场,否则实盘 ±2% 止盈会拉平差异、消融失效(已知近似:实盘经 bracket 离场后,后续利空找不到实盘持仓,出场变体此后只靠自身规则离场)。前端「消融实验」页(`/api/shadow`)展示各组合的净值曲线(窗口起点归一)、汇总对比与最近影子成交。
...
参照 [TradingAgents 论文](https://arxiv.org/abs/2412.20138)的验证方法:在「策略回测」标签页选定最多 5 只股票和一段历史窗口(≤366 天),系统抓取窗口内的全部历史新闻、**用与实盘完全相同的分析师 prompt 逐篇重新分析**(时效分锚定在发布时刻),沿实盘同一套可交易门槛推导信号,再按实盘交易规则(±2% 止损止盈、48h 持有上限、同票利好刷新、利空即卖)在日线上撮合出「AI 新闻策略」曲线,与五个经典量化基线同窗对比:

| 策略 | 规则(参数见 `config.backtestParams`) |
|------|------|
| 买入持有 | 窗口首日收盘全仓买入持有到期末 |
| MACD | DIF−DEA 柱线转正持仓、转负空仓(12/26/9) |
| KDJ+RSI | 超卖进场(J≤20 或 RSI≤30)、超买离场(J≥80 或 RSI≥70) |
| ZMR 零均值回归 | 价格显著低于 20 日均值(z≤−1)买入,回归均值(z≥0)卖出 |
| SMA 双均线 | 快线(10)上穿慢线(30)持仓、下穿空仓 |

指标输出与论文一致:累计收益、年化收益、夏普比率、最大回撤,另加交易次数与胜率(Wilson 95% CI)。所有策略单标的独立账本、多头全进全出、默认零交易成本(`BACKTEST_COST_BPS` 统一可配),基线在收盘决定、次日收盘执行(严格因果)。日线自动向窗口前多取约 40 根做**指标暖机**(MACD 需 35 根、SMA 慢线需 30 根才有首个信号),因此即便只回测一个月,技术基线也能从窗口首日给出信号;交易与净值曲线仍严格限定在所选窗口内。
...
- **指数指标**:纳指 / 标普 / VIX 的点位与日涨跌、距 200 日均线、52 周回撤与区间位置、RSI(6)、TTM PE 及历史分位的指标矩阵;
- **情绪与说明**:六张**极端买入信号**卡(VIX >30 / 恐惧贪婪 <25 / RSI(6) <22 / PE 历史分位 <20% / 52 周回撤 ≤−20% / 25 日急跌 ≤−12%),三态展示——绿=触发(极端买入)、黄=接近(统一为触发阈值的 75% 逼近带)、灰=未触发;无论数值高(VIX 飙升)或低(RSI 超卖),同属「买入机会」逻辑即同色。下方是「过去六个月 VIX 与 PE 分位」月度图(VIX 月均及其近 5 年分位由日线现算,PE 分位线随每日快照逐步积累);
- **策略与纪律**:五档定投倍数(0.25×–2×)——任一信号触发 → 2×,接近或情绪偏谨慎(恐惧贪婪 <45)→ 1.5×,估值分位 ≥85% / ≥70% → 0.25× / 0.5×,否则 1× 常态基准;附今日市场结论(估值/情绪/趋势/操作建议)与静态操作纪律清单。

数据源:指数与 VIX 走既有行情源;恐惧贪婪与指数 PE 分位来自**公开免密钥接口**(fail-open:失败先回退 48 小时内旧值,超龄该卡显示「暂无数据」,其余卡不受影响)。服务端每日美东收盘后(`VALUATION_REFRESH_ET_HOUR`,默认 16 点)自动刷新并把当日指标 upsert 进 `valuation_snapshots`(033 迁移;缺表只停历史积累);页面访问超过 `VALUATION_CACHE_MINUTES`(默认 30 分钟)现算,盘中数据保持基本新鲜。管理重置**保留**本表(外部市场数据,沿 `symbol_reference` 先例)。
...
- **前端**: React + Vite + [Ant Design 5](https://ant.design/)(深色/浅色双主题、默认深色、专业金融终端设计语言、中文界面、移动端自适应),净值曲线用 [lightweight-charts](https://github.com/tradingview/lightweight-charts),其余图表 Recharts(构建后由后端静态托管)。字体自托管(Archivo / JetBrains Mono)。配色遵循美股惯例:绿涨红跌
...
| `ENABLE_SEC_FILINGS` | `true` | 是否启用 SEC EDGAR 8-K 监管文件源(最高可信档 1.00,全市场实时流;026 迁移记录文件元数据,未迁移自动降级) |
| `SEC_USER_AGENT` | 仓库标识 | SEC 公平访问政策要求申明的 User-Agent,建议改成含联系方式的自有标识 |
| `SEC_MAX_FILINGS_PER_POLL` | `20` | 每轮最多抓取多少份新 filing 的正文 |
| `SEC_8K_ITEM_WHITELIST` | 重大条目集 | 进入分析管线的 8-K 事项条目白名单,名单外的例行披露直接丢弃(设空=不过滤) |
| `ENABLE_SYMBOL_REFERENCE` | `true` | 标的名录:交易所官方符号目录做准入门的名录存在性(OTC/退市)/测试标的/财务异常/ETF/交易所校验;镜像入 symbol_reference 表(028 迁移,缺表只停镜像) |
| `SYMBOL_DIRECTORY_REFRESH_HOURS` | `6` | 名录刷新间隔(小时) |
| `ENABLE_HALT_GUARD` | `true` | 停牌守护:官方停牌 feed 盘中轮询——停牌股禁新买入、持仓暂停止损/止盈、利空卖单挂队列复牌成交 |
| `HALT_POLL_SECONDS` | `60` | 停牌轮询间隔(秒,休市自动跳过;数据陈旧时守护自动放行) |
| `ENABLE_REFLECTION` | `true` | 平仓后是否复盘并沉淀经验教训(注入后续决策),需执行 006 迁移 |
| `BACKTEST_MAX_SYMBOLS` | `5` | 策略回测:单轮标的数上限(032 迁移,缺表功能停用) |
| `BACKTEST_MAX_DAYS` | `366` | 策略回测:单轮窗口跨度上限(天) |
| `BACKTEST_MAX_ARTICLES` | `2000` | 策略回测:单轮历史新闻总量上限(成本护栏,未命中缓存的文章每篇一次 LLM 调用) |
| `BACKTEST_LLM_CONCURRENCY` | `3` | 策略回测:历史新闻重析的 LLM 并发数 |
| `BACKTEST_COST_BPS` | `0` | 策略回测:每笔双边固定交易成本(基点,对所有策略一致;0=论文口径) |
| `ENABLE_VALUATION_BOARD` | `true` | 估值看板:估值/情绪监控页签(纯展示观察层;033 迁移缺失只停历史图线) |
| `VALUATION_REFRESH_ET_HOUR` | `16` | 估值看板每日自动刷新触发时间(美东 24 小时制,收盘后当日一次并落快照) |
| `VALUATION_CACHE_MINUTES` | `30` | 估值看板载荷的进程内缓存时长(分钟),页面访问超龄现算 |
| `FEAR_GREED_URL` / `INDEX_VALUATION_URL` | 内置公开接口 | 估值看板的外部数据源地址(可选覆盖,返回 JSON 结构需兼容;失败仅对应卡片降级) |
...
| `BROKER_MIRROR_MAX_RETRIES` | `3` | 镜像单限价重挂次数上限(027;`0`=关闭重挂;卖单在限价重试耗尽后仍升级一次市价单保证收敛) |
| `BROKER_MIRROR_BUY_RETRY` | `chase` | 买单追单策略:`chase`=限时追单(漂移上限复用 `BUY_PRICE_DRIFT_ABORT_PERCENT`),`off`=过期即放弃 |
| `BROKER_MIRROR_RECONCILE` | `true` | 对账清理:定期(约 30 分钟)平掉"券商持有但对账基准账本(内部账本/绑定影子变体)已不持有"的滞留仓位 |
| `BROKER_MIRROR_REPRICE_MINUTES` | `15` | 陈旧在途限价单主动撤单重挂(分钟,`0`=关闭):超时未成交的镜像限价单主动撤掉,由重挂链按实时价重挂 |
| `BROKER_SNAPSHOT_SECONDS` | `30` | 净值对照快照间隔(秒,下限 10);休市时段净值不动,自动降回 10 分钟一条 |
...
| `GET /api/macro` | 宏观环境(当前 regime 与生效参数、宏观事件、经济日历/黑窗;`?date=YYYY-MM-DD` 回看历史日:regime 按当日事件回溯推算、事件/日历限定该日) |
| `GET /api/macro/history` | 宏观环境历史序列(近 N 个美东日的逐日 regime/风险分,热力图数据源;`?days=` 默认 120,服务端缓存 10 分钟) |
| `GET /api/valuation` | 估值看板(指数指标、六项极端买入信号三态、五档定投倍数建议、今日结论、月度 VIX/PE 分位序列;外部源失败对应字段为 null) |
