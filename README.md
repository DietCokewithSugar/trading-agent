...
每次买入时,服务端按**固定百分比**设定止损价与止盈价:成本价下方 `STOP_LOSS_PERCENT`(默认 2%)止损、上方 `TAKE_PROFIT_PERCENT`(默认 2%)止盈——买入价变动 ±2% 即卖出,窄敞口快进快出。LLM 决策中的止损建议仅存入决策回放记录,不再影响实际成交。服务端每 `RISK_CHECK_SECONDS`(默认 30 秒)监控一次持仓价格(含盘前盘后),跌破止损价或触及止盈价即自动全仓卖出,交易记录中会标注「自动止损 / 自动止盈」及详细原因。加仓时按新的平均成本重新设定。在本功能上线前已存在的持仓没有止损价,可在 Supabase 中手动 `update positions set stop_loss=..., take_profit=...` 补设。

**波动自适应敞口**(023 迁移,默认**关**,管理员页运行时开关):固定 ±2% 在不同波动的票上不是同一个 bracket——高波动小票几小时内被噪声随机扫损,低波动大票走满时限。开启后每笔买入的敞口按该股 20 日已实现日波动缩放:`bracket = clamp(BRACKET_VOL_K × vol20, BRACKET_MIN_PERCENT, BRACKET_MAX_PERCENT)`(默认 1×,1.5%–4%,对称),波动取数失败的单笔自动回退固定值。开关在管理员页(#/admin)「交易策略」选择器中切换为 `vol_bracket` 并持久化(024 起策略选择器取代原布尔开关),**没有 ENABLE_VOL_BRACKET 环境变量**;每笔成交的 bracket 宽度与波动快照落 `trades.stop_loss_percent/take_profit_percent/bracket_vol`(开关关闭时也记录波动,供统计层跨票归一)。关闭期间,消融实验中的 `vol_bracket` 影子变体持续按该规则跑对照,跑赢实盘时参数顾问会在管理页建议切换。

**主账户交易策略选择器**(024 迁移,管理员页):策略预设与消融实验的影子变体一一对应——`default`(候选池+LLM 决策,±2%/48h)/ `wide_bracket`(±4%/96h)/ `trailing_only`(仅移动止损不设止盈)/ `vol_bracket`(波动自适应)/ `immediate_trade`(信号即时成交)/ `immediate_rotation`(即时成交+止盈腾位)/ `equal_weight`(等权)。入场路径类(后三个)绕过候选池、LLM 决策与风控官(资金分配器同时暂停),但锁内硬风控(暂停开关/当日亏损熔断/持仓帽/行业集中度/预算钳制等)对所有策略始终生效。先看实验里哪个变体持续跑赢,再切实盘策略——参数顾问会自动给出对应建议。

**展示主账本切换**(024 迁移,管理员页):开启后仪表盘主视图(净值/持仓/净值曲线)切换为**券商模拟账户**的真实撮合数据(盈亏基线为最早一条对照快照净值,券商取数失败自动回退内部账本);交易引擎、绩效统计与决策仍完全基于内部模拟账本。需已配置券商模拟账户 API key。

**移动止损**(007 迁移,`ENABLE_TRAILING_STOP` 默认**关**):开启后股价创出建仓后新高时,止损价按买入时设定的止损距离跟随上抬(峰值价 × (1 − 原止损距离)),只升不降、止盈价不动。固定 ±2% 语义下默认关闭——实际卖点应严格是"买入均价 ±2%",而非距峰值回撤。消融实验中的 `trailing_only` 影子变体(不设止盈、仅棘轮止损)为它持续积累证据。

### 持仓时限与利好/利空联动(020)

高频轮换的核心机制(需执行 020 迁移,`MAX_HOLD_HOURS=0` 关闭):

- **48 小时持有上限**:任何持仓最长持有 `MAX_HOLD_HOURS`(默认 48)小时,到期由风控循环全仓强制平仓(交易标注「持有超时」),休市到期的在下一可交易时段(含盘前 04:00)首个检查平仓;
- **同票新利好刷新**:持有期间出现**新的不同**利好事件(一/二档、经事件级去重与相似度判定,同一事件的重复报道不算)→ 持有时钟刷新回 48 小时,同时止盈线上抬 `TAKE_PROFIT_STEP_PERCENT`(默认 1)个百分点(逐事件累加);持有票的利好不再入候选池加仓;
- **同票新利空即清仓**:持有期间出现新的利空事件(一/二档)→ **不经 LLM 决策**,确定性立即全仓卖出(休市挂开盘队列);
- **止盈腾位(选盈利票)**:资金分配器的优质候选因「持仓数已满 / 现金保留 / 总敞口」被拒时,自动在当前**盈利**持仓中选一个现价最接近止盈价的全仓止盈(交易标注「止盈腾位」),腾出容量后立即重试该候选(每轮分配最多腾位一次);无盈利持仓时候选照常留池等待。
...
- **开盘队列**(010 迁移):休市时段(夜间/周末/假日)产生的信号**不再按上一收盘价立即成交**——真实世界里隔夜新闻只能在下一时段成交,隔夜跳空动辄数个百分点,按 stale 价成交等于把市场兑现的跳空记成策略收益。这类信号挂入 `pending_orders` 表(交易页可见「待开盘订单」),下一个可交易时段成交:盘前/盘后按**实时盘外成交价**,常规时段按**当日开盘价 + 盘中滑点**;超过 `PENDING_ORDER_MAX_AGE_HOURS`(默认 96 小时,覆盖周末长假)未成交自动作废。盘前盘后有真实成交价,信号仍立即成交不入队。
...
内部滑点模型再精细也是"自己给自己判卷"。配置 [Alpaca Paper Trading](https://app.alpaca.markets/signup)(注册仅需邮箱,无 KYC、无入金、免费)的 API Key 后,实盘每笔成交会同步以 **marketable 限价单**(买=成交价×(1+`BROKER_MIRROR_LIMIT_SLACK_PERCENT`),卖=×(1−),当日有效,盘前盘后自动带盘外标记)镜像到券商模拟账户,由**真实盘口(NBBO)撮合**——含随机部分成交与真实订单生命周期。后台每分钟回填撮合结果:

- **逐笔偏差**:券商成交价 vs 内部账本成交价(bps,正值=对我们不利,即内部模型偏乐观);
- **净值对照**:券商账户 equity vs 内部组合净值,每 10 分钟一条快照;
- 长期未成交/过期的对照单本身就是信号——该价格在真实盘口拿不到。

「消融实验」页的"券商模拟对照账本"卡片展示以上统计(`GET /api/broker-mirror`)。纯观测层:不配置 key 或未执行 021 迁移时整体停用,绝不影响交易主链路;管理重置会同步撤单、清空券商模拟持仓。注意免费数据档为单一交易所盘口(IEX),小盘股撮合价代表性有限;准入门槛(市值/成交额)已过滤掉大部分受影响标的。

### 多券商模拟账户 · 消融实验真实撮合(025)

021 只镜像实盘账本;025 把同一套"真实盘口判卷"扩展到任意消融变体:管理员页(#/admin)可**添加多个券商模拟账户**(API key 先经连通性校验再入库;密钥只进不出、`broker_accounts` 表 RLS 无公共读、key 脱敏展示),并给每个账户**指派一个消融变体作为用途**(同一变体最多绑定一个账户)。指派后,该变体的每笔影子成交都以 marketable 限价单同步发往对应账户(幂等键 `shadow-{影子成交 id}`;卖出按券商侧持仓调量,碎股被拒退整数股)。后台每 5 秒回填在途执行单的撮合结果(无在途单时零券商请求),每账户 60 秒落一条净值快照(对照同变体影子净值);管理页展示的账户净值为**实时取数**(5s 缓存)。账户净值与影子净值的偏离 = 该策略经真实撮合检验后的水分。管理重置会对各账户撤单、清仓并清空执行单/快照(账户与指派保留)。
...
- 后台任务自动回填三个口径的前瞻收益:信号后 1 小时(实时价,休市窗口错过则缺省)、信号日后第 1 / 第 5 个交易日收盘价。1d/5d 按**两条独立到期队列**回填(1d 满 24 小时、5d 满 7 天才进队,最老优先),查不到日线的股票(退市/停牌)进入冷却名单不再空耗每轮预算——避免"毒丸行"把回填吞吐拖死;
- 统计样本只计入**至少回填了一个口径**的有效信号——刚产生、前瞻收益尚未到期的新信号不占 5000 条样本预算,覆盖窗口按有效数据计;
- **实盘兑现**(±2%/48h 策略口径):每笔平仓按"同票最近一笔买入"回溯源信号,按档位/来源分桶统计 止盈/止损/持有超时 的离场分布、胜率(Wilson 95% CI)与已实现盈亏——止盈占比高说明该类信号强度足以在时限内兑现 +2%,止损过半说明信号与固定敞口不匹配,这是当前策略下最直接的信号质量度量(1h/1d 为决策口径,5d 降级为研究口径);
...
- **时间窗口与样本透明**(017 后续):页面可切 7 天 / 30 天 / 全部(`?days=`),后端分页拉取突破单次 1000 行上限(全量超过 5000 条采样上限时截断并明示实际覆盖的起点);每个命中率都带 **Wilson 95% 置信区间**,且仅在区间整体高于/低于 50% 时才着色——区间跨过 50% 说明样本不足以下结论,防止 20 条样本就调参;
...
- **最大持仓数** `MAX_OPEN_POSITIONS`(默认 15):开新仓受限,加仓不受限;
...
- **环境联动的组合参数**(代码强制,只约束买入):每个状态对应一组 当日买入预算 / 现金保留下限 / 持仓总敞口上限 / 买入金额乘数 / 允许档位——避险时只做一档高置信信号且仓位减半,宏观冲击时暂停一切新开仓(卖出/止损永不受限);另有当日开新仓数配额 `MAX_NEW_POSITIONS_PER_DAY`(默认 0=关闭,高频轮换模式下新仓节奏由预算钳制与现金约束控制;加仓不计);
- **候选池 + 资金分配器**:利好信号过准入门槛后**不再立即交易**,而是进入 `candidate_signals` 候选池;休市(夜间/周末/假日)只入池持续排序,**可交易时段(盘前 04:00 至盘后 20:00,美东)每 `ALLOCATION_INTERVAL_MINUTES`(默认 15)分钟一轮,当日首轮(盘前开始)立即清算隔夜候选**——按 档位 × 置信度 × 时效衰减 × 来源可信度 × 宏观乘数 × 行业乘数 打分排序,每轮只对前 `MAX_ALLOCATIONS_PER_RUN`(默认 3)个候选发起 LLM 交易决策(成本反而低于逐条即时决策)。资金不足的高分信号标记「资金受限」留池,卖出释放现金后自动复评;超过 `CANDIDATE_MAX_AGE_HOURS`(默认 24)小时过期;
...
**全时段交易与多空不对称**:资金分配器覆盖盘前(美东 04:00)至盘后(20:00)的全部可交易时段——利好信号仍**一律入池**由分配器统一排序,但盘前/盘后同样按分配节奏执行(盘外成交价 + ×3 时段滑点),只有真正休市(夜间/周末/假日)的候选才积累到次日盘前首轮清算;而**利空仍即时卖出**(盘前盘后按 aftermarket 价,休市经开盘队列)。保守不对称的理由不变:买入是主动加风险,值得等统一的资金分配排序;卖出是降风险,一刻也不该等。利好的执行延迟(最多一个分配间隔、隔夜到盘前)这笔"排队成本"被显式度量——入池时记录市场价(`candidate_signals.entry_price`),成交时落库 入池→成交价格漂移 与 等待分钟数(`trades.pool_*`,016 迁移),「信号质量」页按执行路径与等待时长分桶对比前瞻收益,延迟换来的更优分配是否抵得过信号变陈旧,用数据回答。
...
| `wide_bracket` | 出场规则(敞口宽度) | 1:1 跟随实盘买入,止损/止盈放宽到 **±4%**、持有上限 **96h**——检验固定 ±2%/48h 是否过窄(噪音扫损) |
| `trailing_only` | 出场规则(固定止盈) | 1:1 跟随实盘买入,初始止损同实盘距离但**不设止盈**,移动止损棘轮只升不降——检验固定止盈是否截断利润 |
| `vol_bracket` | 出场规则(固定宽度) | 1:1 跟随实盘买入,敞口按该股 **20 日波动自适应**(1.5%–4%)——实盘开关开启前的对照实验 |
| `immediate_trade` | 候选池 + LLM 决策链 | 独立组合:可交易利好信号到达**即按确定性仓位买入**(档位/置信度/来源缩放),利空清仓;休市信号顺延至下一可交易时段按实时盘外价成交 |
| `immediate_rotation` | 候选池 + LLM 决策链(+满仓机会成本) | 独立组合:同「信号即时成交」,但现金不足时先**全仓止盈最接近止盈价的盈利持仓**腾出资金再重试——与 immediate_trade 对比隔离止盈腾位机制的净贡献(024) |
| `equal_weight` | LLM 仓位分配 | 独立组合:可交易信号一律按固定比例(默认 5%)等权买入;休市信号同样顺延 |
| `*_rotation`(六个) | 满仓机会成本(逐变体) | **腾位对照组(025)**:上面每个消融变体各配一个同源影子(`no_risk_officer_rotation` / `no_macro_filter_rotation` / `wide_bracket_rotation` / `trailing_only_rotation` / `vol_bracket_rotation` / `equal_weight_rotation`;`immediate_trade` 的即 `immediate_rotation`)——同一信号源/仓位/出场规则,唯一差异是现金不足时先**全仓止盈最接近止盈价的盈利持仓**再重试买入一次(`trailing_only` 系无止盈线,退化为浮盈比例最高者);本体 vs 腾位版的同窗差值即「止盈腾位」机制的净贡献 |
| `spy_benchmark` | — | 启用时一次性全仓买入 SPY 持有 |
| `cash` | — | 纯现金 |

若「无风控官」长期跑输实盘,说明风控官的否决在创造价值;若「无宏观过滤」跑赢,说明宏观层过度拦截;「信号即时成交」对比实盘量化候选池+LLM 决策链的净价值;「信号等权」对比实盘检验 LLM 仓位是否有效。**三个出场消融变体(023)补上了此前唯一没被消融的层**:实盘的信号驱动卖出(新闻利空/持仓复查/止盈腾位)照样镜像给它们,但实盘自身的止损/止盈/持有超时卖出**不镜像**——出场变体按自己的规则离场,否则实盘 ±2% 止盈会拉平差异、消融失效(已知近似:实盘经 bracket 离场后,后续利空找不到实盘持仓,出场变体此后只靠自身规则离场)。前端「消融实验」页(`/api/shadow`)展示各组合的净值曲线(窗口起点归一)、汇总对比与最近影子成交。
...
- 查看**参数建议**(`/api/admin/advisor`):把近 30 天的信号统计反向映射成参数调整建议——低可信来源利好收益为负 → 提高 `MIN_FINAL_CONFIDENCE`;新闻稿利好命中率显著低于 50% → 加大 `PRESS_BULLISH_PENALTY` 折价;第 2 档 1d 命中率显著高于第 1 档 → 档位/LLM 分档需校准;被宏观过滤/风控官否决/资金受限/冲突搁置拦下的信号持续上涨 → 对应层过度保守(下跌则确认该层在创造价值);置信度刻度倒挂 → analyst prompt 校准;**实盘兑现规则**(±2%/48h 口径):平仓中止损占比过半 → 信号与固定敞口不匹配(收紧入场门槛或复核敞口),止盈过半 → 确认匹配良好,低可信来源持仓过半被止损打出 → 加深来源折价。每条建议都带样本量与 95% 置信区间证据,**小样本或差异不显著时规则保持沉默**;影子组合(017)运行满 14 天后还会给出消融对照(关闭某层的变体显著跑赢/跑输实盘)。建议仅供参考,参数变更仍由人工通过环境变量执行;
...
> **架构文档**:全量系统架构(每个文件的职责与核心逻辑、调度循环表、数据库表清单、横切约定)见 [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md)。
...
| `STOP_LOSS_PERCENT` / `TAKE_PROFIT_PERCENT` | `2` / `2` | 固定止损/止盈百分比(相对买入均价,代码强制,LLM 建议被覆盖):跌 N% 止损、涨 N% 止盈,触及即全仓卖出 |
| `TAKE_PROFIT_STEP_PERCENT` | `1` | 同票新利好(一/二档、经去重)刷新持有时钟时止盈线上抬的百分点(0=只刷新时钟) |
| `MAX_HOLD_HOURS` | `48` | 持仓最长持有小时数,超时全仓强制平仓(新利好/买入成交刷新时钟);设 0 关闭,需执行 020 迁移 |
| `ENABLE_TRAILING_STOP` | `false` | 移动止损:创新高后止损价跟随上抬,需执行 007 迁移;固定 ±2% 语义下默认关闭 |
| `BRACKET_VOL_K` / `BRACKET_MIN_PERCENT` / `BRACKET_MAX_PERCENT` | `1` / `1.5` / `4` | 波动自适应敞口区间常量(023):bracket = clamp(k×20日波动, min%, max%)。开/关是管理员页运行时开关,无环境变量 |
...
| `MAX_OPEN_POSITIONS` | `15` | 硬风控:最大同时持仓数(加仓不受限);设 0 关闭 |
...
| `ALLOCATION_INTERVAL_MINUTES` | `15` | 资金分配器节奏:可交易时段(盘前 04:00–盘后 20:00 美东)每 N 分钟一轮,当日首轮(盘前开始)立即清算隔夜候选 |
| `MAX_ALLOCATIONS_PER_RUN` | `3` | 每轮分配最多执行的买入候选数(LLM 决策只对头部候选发生) |
| `CANDIDATE_MAX_AGE_HOURS` | `24` | 候选有效期(小时),超龄未分配自动过期 |
| `MAX_NEW_POSITIONS_PER_DAY` | `0` | 当日最多开新仓数(加仓不计);默认 0=关闭(高频轮换模式) |
...
| `GET /api/broker-mirror` | 券商模拟对照账本(逐笔成交价偏差统计、账户净值对照、最近对照单;未配置返回 enabled:false) |
...
| `POST /api/admin/strategy` | 管理:主账户交易策略选择(body `{"strategy":"default|wide_bracket|trailing_only|vol_bracket|immediate_trade|immediate_rotation|equal_weight"}`,024) |
| `POST /api/admin/primary-ledger` | 管理:展示主账本切换(body `{"enabled":true|false}`,开=仪表盘主视图展示券商模拟账户,024) |
| `GET /api/admin/broker-accounts` | 管理:券商模拟账户总览(脱敏账户列表 + 实时净值 + 成交统计 + 可指派用途,025) |
| `POST /api/admin/broker-accounts` | 管理:新增券商模拟账户(body `{"name","key_id","secret_key","base_url"?}`,先连通性校验后入库,025) |
| `POST /api/admin/broker-accounts/:id` | 管理:更新账户(body `{"name"?,"purpose"?,"enabled"?}`,purpose=影子变体名或 null;同变体重复指派 409,025) |
| `DELETE /api/admin/broker-accounts/:id` | 管理:删除账户(先尽力撤销券商侧未成交单,025) |
