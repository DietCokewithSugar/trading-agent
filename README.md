...
经聚合 API 转发的文章在原始来源分上再扣 0.03 的聚合折价。在此之上计算**综合置信度** = 来源可信度 × 分析置信度 × 时效分(1 小时内 1.0,24 小时以上 0.5)× 事件档位分:
...
- 前端「信号质量」页(`/api/signal-stats`)汇总:按**事件档位 / 来源可信度 / 分析置信度 / 已交易 vs 未交易 / 入池时宏观环境 / 执行路径与排队时长**分桶的方向命中率与平均前瞻收益(按信号方向调整,利空跌了算命中),以及综合置信度与收益的相关系数(IC)、置信度校准(置信度分桶命中率是否单调上升);
...
| `GET /api/stream` | SSE 实时推送流(news / analysis / trade / portfolio / snapshot / cycle / macro) |
