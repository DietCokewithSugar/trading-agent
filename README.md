...
**未成交跟进(027)**:镜像单未成交不再一弃了之(否则卖单过期会在券商账户滞留孤儿持仓、买单过期永不建仓,两账本永久分歧)——休市时段的镜像单先落"待开盘"(deferred),开盘后以实时价挂单;在途单过期/被撤后按策略重挂(`BROKER_MIRROR_MAX_RETRIES`,偏差基线始终是原内部成交价,追单成本如实入账):**卖单保证收敛**(限价重挂耗尽后升级一次市价单),**买单限时追单**(`BROKER_MIRROR_BUY_RETRY=chase`,上行漂移超过 `BUY_PRICE_DRIFT_ABORT_PERCENT` 或次数用尽即放弃,不以离谱价格追高污染对照数据);另有约每 30 分钟一轮的**对账清理**(`BROKER_MIRROR_RECONCILE`),自动平掉"券商持有但内部账本已不持有"的滞留仓位。
...
| `BROKER_MIRROR_RECONCILE` | `true` | 对账清理:定期(约 30 分钟)平掉"券商持有但内部账本已不持有"的滞留仓位 |
