...
**波动自适应敞口**(023 迁移,默认**关**,管理员页运行时开关):固定 ±2% 在不同波动的票上不是同一个 bracket——高波动小票几小时内被噪声随机扫损,低波动大票走满时限。开启后每笔买入的敞口按该股 20 日已实现日波动缩放:`bracket = clamp(BRACKET_VOL_K × vol20, BRACKET_MIN_PERCENT, BRACKET_MAX_PERCENT)`(默认 1×,1.5%–4%,对称),波动取数失败的单笔自动回退固定值。开关在管理员页(#/admin)切换并持久化,**没有 ENABLE_VOL_BRACKET 环境变量**;每笔成交的 bracket 宽度与波动快照落 `trades.stop_loss_percent/take_profit_percent/bracket_vol`(开关关闭时也记录波动,供统计层跨票归一)。关闭期间,消融实验中的 `vol_bracket` 影子变体持续按该规则跑对照,跑赢实盘时参数顾问会在管理页建议开启。
