...
在同一进程内跑十余个 `setInterval` 后台循环(新闻轮询/报价推送/快照/风控/复查/开盘队列/
前瞻收益回填/经济日历/宏观重算/资金分配/市场核验/影子止损/券商对照与账户轮询)。
没有独立队列或 worker。
...
| 券商账户轮询 `pollBrokerAccountOrders` | 5s | 无条件注册(无账户/缺 025 表时零成本返回);回填多账户在途执行单(marketable 限价单基本秒成,无在途单的 tick 零券商请求)+ 每账户 60s 限频净值快照 |
...
`GET/POST/DELETE /broker-accounts(/:id)`(多券商模拟账户 CRUD,025:列表脱敏+实时净值+成交统计;
新增先连通性校验(失败 400 不入库);purpose 按影子变体白名单校验(cash 排除),同变体重复指派 409)、
...
| `rotation.js` | 止盈腾位纯函数:盈利且有止盈价的持仓中选 `current_price/take_profit` 最大者(排除候选同票);`pickRotationSellByPnl`(025)为无止盈线组合(trailing_only 系影子)的退化规则——选浮盈比例最高的盈利持仓 |
...
| 文件 | 职责与核心逻辑 |
|---|---|
| `shadowEngine.js` | 纯计算:买入金额硬帽(`computeShadowSpend`)、加权均价记账(`applyBuy`/`applySell`)、镜像仓位还原(`unscaleFraction`/`unapplyScale`——把实盘成交"反算"回未被风控官/宏观缩放前的仓位)、宏观拦截候选重放选取(`pickTopBlocked`)、估值(`valuePositions`)、休市顺延队列修剪(`prunePendingSignals`)、腾位选仓输入映射(`toRotationPositions`,025 起带 `unrealized_pnl_percent` 供无止盈线变体的退化规则) |
| `shadowPortfolio.js` | 编排与落库。十个变体:`no_risk_officer`(镜像实盘但还原官审缩仓,官否决的买入按否决前方案照买)、`no_macro_filter`(镜像用宏观钳制前仓位,regime 过滤/冲击/黑窗/结算期宏观拒绝拦下的买入照样重放)、**出场消融三件套(023,出场规则此前是唯一没被消融的层)**:`wide_bracket`(1:1 镜像,±4%/96h)、`trailing_only`(1:1 镜像,初始止损同实盘距离、无止盈,棘轮 `trailing.js#computeTrailedStop` 不依赖 ENABLE_TRAILING_STOP,棘轮锚点 `shadow_positions.peak_price`,缺 023 列停用棘轮告警一次)、`vol_bracket`(1:1 镜像,用 trader 每笔算好的波动 bracket——影子层零额外取数;取数失败回退实盘同宽;实盘开关开启后与实盘趋同=实验完成态);`immediate_trade`(独立:信号到达即确定性建仓,消融的是候选池+LLM 链)、`immediate_rotation`(024,独立:同即时成交,现金不足时先全仓止盈最接近止盈价的盈利仓再重试一次——shadowBuy 返回状态对象,no_spend 触发同一 enqueue 任务内的卖-买两步;与 immediate_trade 对比隔离腾位机制的净贡献)、`equal_weight`(独立:固定 5% 等权,消融 LLM 仓位)、`spy_benchmark`(一次性全仓 SPY)、`cash`。**镜像卖出触发器过滤(023 核心设计)**:信号驱动卖出(news/review/rotation)镜像进全部跟随变体,bracket 驱动卖出(stop_loss/take_profit/max_hold)跳过出场三件套——否则实盘 ±2% 止盈会拉平消融(已知近似:实盘 bracket 离场后,后续利空无实盘卖出可镜像)。关键机制:全部钩子 fire-and-forget 经进程内串行队列;**同一 (variant, analysis_id) 只买一次**(019 唯一部分索引,DB 级硬约束);**零额外 LLM 调用**(确定性仓位+默认止损,是消融近似而非回放);**休市信号顺延**(进程内队列,首个可交易 tick 按实时盘外价清算,96h 超龄作废,重启丢失可接受);`checkShadowStops` 对影子持仓做同口径 止损/止盈/时限 监控(每变体时限 `config.shadowVariantMaxHoldHours`,wide_bracket 96h 其余 48h;spy_benchmark 豁免时限;trailing_only 止损比较前先跑棘轮);快照搭车主循环 10 分钟限频;管理重置时截表重建变体。**腾位对照组(025,`ROTATION_COUNTERPARTS`)**:每个尚无腾位机制的消融变体各配一个 `*_rotation` 影子(`no_risk_officer_rotation`/`no_macro_filter_rotation`/`wide_bracket_rotation`/`trailing_only_rotation`/`vol_bracket_rotation`/`equal_weight_rotation`;`immediate_trade` 沿用 024 的 `immediate_rotation`,共 16 个变体)——同一买卖信号/仓位/出场规则(镜像买卖、官否决重放、宏观钳制与过滤重放全部同步落到对照组,棘轮与 96h 时限按本体规则生效),唯一差异是 `shadowBuyWithRotation`:`no_spend` 时先全仓止盈"最接近止盈价的盈利持仓"(trailing 系无止盈线,退化为浮盈比例最高者 `pickRotationSellByPnl`)再重试一次;本体 vs 腾位版同窗差值 = 止盈腾位机制的净贡献。影子成交落库带回 `id`,`onShadowTrade` 钩子(fire-and-forget)喂给多券商账户执行层(025) |

### 4.5b 券商模拟对照账本(021)与多券商模拟账户(025)

| 文件 | 职责与核心逻辑 |
|---|---|
| `alpacaBroker.js` | 券商模拟账户(Alpaca Paper)REST 薄客户端:下单(限价/day/可盘外)、查单(按 id/幂等键)、账户、单票/全部持仓、撤全单、清全仓;原生 fetch + 15s 超时,错误抛出由调用方降级。`makeBrokerClient({keyId,secretKey,baseUrl})`(025)按账户凭据构建客户端;模块级导出为环境变量凭据的默认客户端(021 用) |
...
| `brokerAccounts.js` | **多券商模拟账户**(025,log 前缀 `[accounts]`):管理员页添加任意多个券商模拟账户(密钥存 `broker_accounts`,RLS 无公开读;新增先经券商连通性校验,失败 400 不入库;secret 永不返回、key_id 脱敏 `maskKeyId`;base_url 仅 https,本地回环放行 http 供联调),每账户指派一个影子变体作为用途(启用+用途部分唯一索引,一变体一账户,冲突 409)。`onShadowTrade` 钩子:该变体每笔影子成交经进程内串行队列以 marketable 限价单发往对应账户(幂等键 `shadow-{影子成交 id}`;卖出按券商持仓调量;碎股 422 退整数股;复用 brokerMirror 的纯函数)。`pollBrokerAccountOrders`(5s):回填在途执行单(无在途单零券商请求)+ 每账户 60s 限频净值快照(对照 `shadow_snapshots` 同变体净值)+ 账户状态回写(ok/error);管理面总览的账户净值走**实时取数**(`liveEquity`,5s TTL 缓存,失败回退最新快照)。管理重置:各账户撤单+清仓,清空执行单/快照,账户与指派保留 |
...
| `broker_accounts` / `broker_account_orders` / `broker_account_snapshots` | 多券商模拟账户(025):账户(**含 API 密钥,RLS 无公开读**;用途=影子变体,启用+用途部分唯一索引)、逐笔影子成交执行单(幂等键 `shadow-{id}`)、每账户净值 vs 影子净值快照 |
| RPC | `execute_trade`(004,原子买卖:行锁、均价重算、按百分比重设止损止盈、现金增减)、`admin_reset_data`(005)、两个快照采样函数 |

RLS 约定:全表启用,公开只读(除 `cycle_runs`/`trade_decisions`/`broker_accounts`);服务端 service_role 全权。
...
025 腾位对照影子变体(无模式变更,资金行启动自动补建)+ 多券商模拟账户三表
(broker_accounts 密钥表 RLS 无公开读;orders/snapshots 公开只读)。
...
| `components/AblationPage.jsx` | 「消融实验」页:各变体净值曲线(窗口归一;腾位对照组沿用本体色相的浅色 + 短虚线区分,025)、汇总对比、行展开(说明/胜率/持仓/成交)、券商模拟对照账本卡(021);页面可见时每 60s 静默刷新估值(影子符号不进 quotes 事件,量大不值盘外双倍配额) |
...
| `components/AdminPage.jsx` | `#/admin`:令牌登录、运行指标、参数建议、决策回放、kill switch、策略选择器、主账本切换、**券商模拟账户管理**(025:添加/删除/启停/用途指派,实时净值 vs 影子净值,10s 自动刷新)、手动触发、数据重置 |
...
| `rotation.test.js` | 止盈腾位选取(含无止盈线组合的浮盈比例退化规则,025) |
...
| `brokerAccounts.test.js` | 多账户纯函数(key 脱敏/入参校验含 base_url 协议约束,025) |
...
1. **日志前缀**按模块:`[cycle] [trader] [risk] [fmp] [event] [api] [scheduler] [admin] [memory] [review] [riskofficer] [queue] [signal] [metrics] [macro] [calendar] [pool] [allocator] [market] [shadow] [decision] [advisor] [quotes] [broker] [accounts] [strategy]`;注释/日志/UI 文案一律简体中文,标识符英文。
