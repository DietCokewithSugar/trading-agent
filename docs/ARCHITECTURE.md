...
`/stats`、`/performance`(vs SPY;二者在券商主账本下由 statsService 切到参照账户口径 —— 净值指标来自镜像快照日度序列、已实现盈亏/胜率来自镜像成交加权均价重放,失败限频回退内部,030)、`/signal-stats`(信号质量)、`/macro`(regime/日历/候选池)、
...
| `bus.js` | 进程内 SSE 总线:客户端集合管理 + `broadcast(event, data)`。事件名:`news`/`analysis`/`trade`/`portfolio`/`snapshot`/`cycle`/`reset`/`macro`/`quotes`/`ledger`(展示主账本切换,030) |
...
| `src/App.jsx` | 顶层路由(hash:主页/#/strategy/#/admin)与全局状态:EventSource 消费十类 SSE 事件(portfolio/snapshot/quotes 全量应用,ledger 触发一次全量刷新原子换源,其余触发定向重取;quotes 经 QuotesProvider 分发,reset 时清空),断线降级轮询,长驻状态封顶(快照 ≤600 点/新闻 ≤600 条) |
