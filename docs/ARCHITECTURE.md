...
`POST /vol-bracket`(波动自适应敞口运行时开关,023;缺列开启时 409)、
...
| `volBracket.js` | 波动敞口运行时开关(023,管理页切换):持久化在 `portfolio_state.vol_bracket_enabled`;开启先写库成功才翻内存(行为开关静默回退会污染证据解读),关闭先翻内存(fail-safe);缺列开启时抛 409 |
