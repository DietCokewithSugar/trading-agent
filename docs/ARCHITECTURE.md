...
`GET|POST|DELETE /broker-accounts[...]`(多券商账户 CRUD,025;新增先校验凭据,列表脱敏)、
...
| `mirrorPolicy.js` | 镜像单跟进策略(027,纯函数):`mirrorLimitPrice`(marketable 限价)、`remainingQty`、`nextRetryClientOrderId`(`-rN` 后缀)、`buyDriftPercent`、`planMirrorFollowUp`(顺延单 等待/提交/放弃 + 终态迁移 重挂/顺延重挂/市价升级/放弃 决策表)、`planReconcile`(对账清理:券商独有全平/超额减仓/在途与内部独有跳过,永不买入对账);IO 全部留在 brokerMirror.js |
| `brokerMirror.js` | 对照账本编排(log 前缀 `[broker]`):实盘每笔成交 fire-and-forget 镜像为 marketable 限价单(限价=内部价±`BROKER_MIRROR_LIMIT_SLACK_PERCENT`,盘前盘后带 extended_hours,幂等键 `trade-{id}`);卖出以券商侧持仓为准(`adjustSellQty`);碎股被拒退整数股。**未成交跟进(027)**:休市时段不直接提交(当日限价单必然过期且限价基于过期报价),落 `deferred` 行由轮询在开盘后以实时价挂单(买单开盘漂移超限 → `abandoned`);在途单迁移到 expired/canceled/rejected 且有余量时按 `planMirrorFollowUp` 生成子行重挂(`attempt`/`retry_of` 记账,幂等键加 `-rN` 后缀,偏差基线始终是原内部成交价;卖单限价重试 `BROKER_MIRROR_MAX_RETRIES` 次后升级一次市价单保证收敛,买单限时追单——上行漂移超 `BUY_PRICE_DRIFT_ABORT_PERCENT` 或次数用尽放弃;休市观察到的迁移先落 deferred 子行;报价/持仓查询失败本轮不写终态,下轮重放整个迁移,崩溃任意点靠幂等键安全重放);每 30 轮轮询(约 30 分钟)入队一次**对账清理**(`reconcileSweep`,`BROKER_MIRROR_RECONCILE`):平掉"券商持有但内部账本已不持有/超额持有"的仓位(仅实盘镜像账户;平仓单 `reconcile-{SYM}-{ET日期}` 每天每票至多一次,后续收敛交同一套重挂机制),走串行队列排在既有镜像提交之后消除竞态。轮询循环回填真实撮合结果并算**带方向偏差**(`signedDiffBps`,正值=对我们不利=内部滑点模型偏乐观);`takeBrokerSnapshots` 按 `BROKER_SNAPSHOT_SECONDS`(默认 30s,下限 10s,休市自动降回 10 分钟)写 equity vs 内部净值快照,`getBrokerSnapshots` 取数上限 5000 行并均匀降采样到 ≤600 点保图表可读。纯函数(`signedDiffBps`/`adjustSellQty`/`summarizeMirror`——被 `retry_of` 引用的未成交终态行记 retried 出分母,deferred 按在途计、abandoned 计未成交)可单测。观测层约定:key 缺失/021 表缺失整体停用;缺 027 列时重挂降级停用(顺延/对账仍工作)警告一次;公开载荷不含供应商名;管理重置时券商侧撤单清仓 + 本地清表 |
...
| `news_articles` | 新闻原文(URL 唯一):来源域名与可信度分(009);监管文件元数据 source_type/filing_form/filing_items(026) |
...
1. **日志前缀**按模块:`[cycle] [trader] [risk] [fmp] [event] [api] [scheduler] [admin] [memory] [review] [riskofficer] [queue] [signal] [metrics] [macro] [calendar] [pool] [allocator] [market] [shadow] [decision] [advisor] [quotes] [sec]`;注释/日志/UI 文案一律简体中文,标识符英文。
