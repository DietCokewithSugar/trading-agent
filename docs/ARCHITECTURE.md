...
(估值+持仓,持仓行含进程内 `halted` 停牌标记)、`/trades`、`/news`(服务端过滤/搜索/分页:
...
1. **日志前缀**按模块:`[cycle] [trader] [risk] [fmp] [event] [api] [scheduler] [admin] [memory] [review] [riskofficer] [queue] [signal] [metrics] [macro] [calendar] [pool] [allocator] [market] [shadow] [decision] [advisor] [quotes] [sec] [symref] [halts]`;注释/日志/UI 文案一律简体中文,标识符英文。
