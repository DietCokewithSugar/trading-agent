...
`/pool`(候选池概览:预览行富化现价 + 时段/盘外价字段(session/extended_price/extended_change_percent/change_percent,首屏徽标用)+ 系统离场口径百分比,供"若现在买入"参考与入池价反事实区间);`POST /run-cycle`(外部 cron 用,匿名共享 120 秒冷却,设 `ADMIN_TOKEN` 后要求请求头)。
...
| `src/quotes-context.jsx` | 实时报价上下文:App 持有 SSE `quotes` 载荷,候选池/个股抽屉经 `useLiveQuotes()` 消费(避免层层透传 props) |
...
| `components/SymbolModal.jsx` | 个股抽屉:报价(含盘外)/持仓/事件聚类的分析历史/交易历史。报价 live 优先(SSE `quotes` 覆盖的持仓/池内票实时跳动),未覆盖符号打开期间每 15s 兜底轮询(只换 quote 字段,热力图选中日期不被打断) |
...
| `quotesPush.test.js` | 实时报价推送纯函数(符号去重/截断、载荷字段兜底与剔除) |
